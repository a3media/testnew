...
$from_city = "Уфа"; //город из которого будем ехать
...
$cities_to = $modx->runSnippet('GoogleSheets', $params);
$cities_from = $modx->runSnippet('GoogleSheets', $params_current);
...
foreach ($cities_to as $to_key => $to_city) { //берем каждую строчку гугл таблицы и выполняем
  ...
  foreach ($cities_from as $from_key => $from_city) {
    ...
    if(!$pdo->getArray('prCitiesItem', array('from' => $from_city['pagetitle'], 'to' => $to_city['pagetitle']))){
      ...
      $main = $pdo->getArray('prCitiesItem', array('from' => $from_city['main'], 'to' => $to_city['pagetitle'] ));
      ...
      //id городов
      $res_from = $pdo->getArray('modResource',  array('pagetitle:=' => $from_city['pagetitle'], ));
      $out_item['from_id'] = $res_from['id'];
      $out_item['to_id'] = $main['to_id'];

      //считаем километраж
      $out_price['pr_km'] = $main_price['pr_km'] + $from_city['km'] + $from_city['km'];
      ...
      echo "УЖЕ СУЩЕСТВУЕТ ЗАПИСЬ " . $from_city['pagetitle'] . " - " . $to_city['pagetitle'] . "\n";
      ob_flush();
      flush();
    }
