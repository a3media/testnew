...
$params = array('table' => 'https://docs.google.com/spreadsheets/d/1r9MGw4_E3-EeacCBh3nIUSEnl-qxJQo1R50Eov_YGH8/', 'range' => 'Для МО', 'ignore' => 1);
...
foreach ($cities as $key => $city) {
  ...
  $items = $pdo->getCollection('prCitiesItem', array('from' => $city['main'] ), array('select' => 'url,id,to,from',));

  foreach ($items as $k => $item1) {


    if(!($modx->getObject('prCitiesItem', array('from' => $city['pagetitle'] , 'to' => $item1['to']), array('select' => 'url', )))){
      ...
      $out_price['pr_km'] = $main_price['pr_km'];
      ...
      echo "запись есть " .$city['pagetitle'] ."-" . $item1['from'] . "\n";
      ob_flush();
      flush();
    }
