...
$to_city_list = array('Новосибирск', 'Казань', 'Челябинск', 'Пермь', 'Нижний Новгород', 'Самара', 'Кемерово', 'Воронеж'); //города до которых будем ехать
...
foreach ($to_city_list as $to_city) { //берем каждый город
  foreach ($cities as $key => $city) { //берем каждую строчку гугл таблицы и выполняем
    ...
    if(!$pdo->getArray('prCitiesItem', array('from' => $city['pagetitle'], 'to' => $to_city))){
      if ($to_city !== $city['main']) {
        ...
        echo "До городов в том же регионе не считаем " . $city['pagetitle'] . " - " . $to_city . "\n";
        ob_flush();
        flush();
      }
      ...
      echo "Уже существует запись " . $city['pagetitle'] . " - " . $to_city . "\n";
      ob_flush();
      flush();
    }
