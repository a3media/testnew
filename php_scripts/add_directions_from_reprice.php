<?php
//ПЕРЕСЧЕТ СТОИМОСТИ НАПРАВЛЕНИЙ ДО РАЙОННЫХ ЦЕНТРОВ ИЗ КРУПНОГО ГОРОДА
...
foreach ($cities as $key => $city) { //берем каждую строчку гугл таблицы и выполняем
    ...
    if(!$pdo->getArray('prCitiesItem', array('from' => $from_city, 'to' => $city['pagetitle']))){
      ...
      echo "Уже существует запись, пересчитываем - " . $from_city . " - " . $city['pagetitle'] . "\n";
      ob_flush();
      flush();

      //Пересчитываем направление
      $value = $pdo->getArray('prCitiesItem', array('from' => $from_city, 'to' => $city['pagetitle']));
