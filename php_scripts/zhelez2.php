...
$params = array('table' => 'https://docs.google.com/spreadsheets/d/1r9MGw4_E3-EeacCBh3nIUSEnl-qxJQo1R50Eov_YGH8/', 'range' => 'Для краснодара_1', 'ignore' => 1);
...
foreach ($cities as $key => $city) {
  ...
  $items = $pdo->getCollection('prCitiesItem', array('to' => 'Санкт-Петербург','from' => $city['main'] ));
  ...
  foreach ($items as $k => $item) {
    ...
    if(!$pdo->getArray('prCitiesItem', array('from' => $item['from'], 'to' => $city['pagetitle']))){
      ...
      $out_item['to'] = $item['to'];
      $out_item['url'] = $generator->cleanAlias($city['pagetitle'] .'-'. $item['to']);
      //узнаем ресурс родитель и записываем его айди в базу
